...
def plot_colored_3d_trajectory(all_trajectories, all_magnitudes, mode):
    """
    Plots 3D trajectories where color indicates Residual Intervention Magnitude.
    Blue = Low Intervention (Base Policy Dominant / Copying)
    Red = High Intervention (Residual Policy Active / Editing)
    """
    fig = plt.figure(figsize=(12, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    # Normalize magnitude for colormap (0 to approx max observed, e.g., 1.0)
    # vmin=0 ensures "No Residual" is always blue. vmax=0.8 covers most typical action norms.
    ...
    # Add a thin grey line for context (the rough path) before adding colored segments
    ...
    for i, (traj, mags) in enumerate(zip(all_trajectories, all_magnitudes)):
        ...
        # Color segments based on magnitude at the start point
        # Ensure mags length matches segments (segments = N-1)
        seg_colors = cmap(norm(mags[:len(segments)]))
        
        lc = Line3DCollection(segments, colors=seg_colors, alpha=0.7, linewidth=1.5)
        ax.add_collection3d(lc)
        
        # Mark start (Green) and end (Black)
        if i == 0: # Only label the first one to avoid legend clutter
    ...
    for mode in modes:
        ...
        for i in range(args.num_rollouts):
            ...
            while not done:
                ...
                # Fallback to qpos[:3] if specific EE link is complex to retrieve generically
                ...
                if mode == 'baseline':
                    ...
                    # Calculate Intervention Magnitude (L2 Norm of residual action)
